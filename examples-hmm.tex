...
is an independent event with probability $p$. In this case, the
attacker has the prior knowledge about the distribution of the true
answers. Specifically, the number of individuals contracting the
disease is $0$, $1$, and $2$ with probabilities $(1-p)^2$, $2p(1-p)$,
and $p^2$ respectively. The prior knowledge can be easily specified by
an information state in
Figure~\ref{figure:geometric-mechanism}. Consider the information
...
% explain why this is pufferfish private
