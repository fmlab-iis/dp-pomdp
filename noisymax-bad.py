# to execute: do
# $ PYTHONPATH=<path to python Z3> python noisymax-bad.py
...
# incorrect noisy max return noisy values, not noisy index

def make_OB (n_queries):
    n_combs = 3 ** n_queries
    ret = mx_make (2 * n_combs, 3 + 1, rzero)
    for i in range (n_combs):
        mx_set (ret, i, 3, rone)
        v = decode (n_queries, i)
        obs = max (v)
        mx_set (ret, n_combs + i, obs, rone)
    return ret
